title Vyond Legacy ^(Wrapper: Offline Mod^) Settings Script
...
:error_location
echo Doesn't seem like this script is in the Vyond Legacy ^(Wrapper: Offline Mod^) folder.
goto end
:noerror_location
...
if "!choice!"=="?1" (
	echo When enabled, two extra windows with more info about what Vyond Legacy ^(Wrapper: Offline Mod^) is doing.
...
if "!choice!"=="?2" (
	echo When set to included Chromium, it opens a browser that comes with Vyond Legacy ^(Wrapper: Offline Mod^).
...
if "!choice!"=="?3" (
	...
	if !CUSTOMBROWSER!==y (
		echo This setting defines which browser Vyond Legacy ^(Wrapper: Offline Mod^) launches with when set to use a custom browser.
...
if "!choice!"=="?4" (
	...
	echo This is automatically disabled when Vyond Legacy ^(Wrapper: Offline Mod^) launches and finds all dependencies.
...
if "!choice!"=="?7" (
	echo By default, Discord rich presence is enabled.
        echo:
	echo It's used to show when you're using Vyond Legacy ^(Wrapper: Offline Mod^)
        ...
        echo:
	echo Turning this off will make Vyond Legacy ^(Wrapper: Offline Mod^) stop saying
...
if "!choice!"=="?8" (
	echo By default, Vyond Legacy ^(Wrapper: Offline Mod^) uses the included VFProxy
	...
	echo:
	echo Toggling this setting will make it so Vyond Legacy ^(Wrapper: Offline Mod^) no
...
if exist "server\characters\characters.zip" (
	...
	if "!choice!"=="?11" (
		echo When first getting Vyond Legacy ^(Wrapper: Offline Mod^), all non-stock characters are put into a single zip file.
...
if "!choice!"=="?10" (
	echo Vyond Legacy ^(Wrapper: Offline Mod^) is free and open-source, and a lot of folks in the community like to make mods for it.
	echo:
	echo Turning on developer mode will provide you with some useful features for development or making your own
	echo mods for Vyond Legacy ^(Wrapper: Offline Mod^), mostly the mods having to do with the batch script.
	echo:
	echo The developer settings will be visible both in these settings and in the Vyond Legacy ^(Wrapper: Offline Mod^) launcher.
	goto reaskoptionscreen
)
:: Dev options
...
echo:
echo:
echo Press 1 to use Vyond Legacy ^(Wrapper: Offline Mod^)'s included Chromium (recommended)
