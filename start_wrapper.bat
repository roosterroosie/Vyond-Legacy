...
set WRAPPER_BLD=72
title Vyond Legacy v%WRAPPER_VER% ^(build %WRAPPER_BLD%^) [Initializing...]
...
if not exist "utilities\checks\disclaimer.txt" (
	echo DISCLAIMER
  echo:
	echo Vyond Legacy is a project to preserve the original GoAnimate flash-based themes.
	echo We believe they should be archived for others to use and learn about in the future.
	echo All business themes have been removed and the props and backgrounds are in import, please use Vyond Studio if you wish to get those.
	...
	echo:
	echo Excluding Adobe Flash and GoAnimate Inc's assets, Vyond Legacy is free/libre software.
	...
	echo:
	echo By continuing to use Vyond Legacy, you acknowledge the nature of this project, and your right to use it.
	echo If you object to any of this, feel free to close Vyond Legacy now.
	echo You will be allowed to accept 20 seconds after this message has appeared.
	echo: 
	PING -n 21 127.0.0.1>nul
	echo If you still want to use Vyond Legacy, press Y. If you no longer want to, press N.
	:disclaimacceptretry
...
title Vyond Legacy v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) [Checking dependencies...]
...
title Vyond Legacy v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) [Installing dependencies...]
		...
		if /i not !ERRORLEVEL!==0 (
			...
			if !FLASH_DETECTED!==n (
				if !NODEJS_DETECTED!==n (
					echo Vyond Legacy needs to install Flash and Node.js.
				) else (
					echo Vyond Legacy needs to install Flash.
				)
			) else (
			...
			echo:
			echo Close this window and re-open Vyond Legacy as an Admin.
...
if !FLASH_DETECTED!==n (
	...
	if !BROWSER_TYPE!==n (
		...
		echo Vyond Legacy will not be able to install Flash.
	...
	echo Vyond Legacy will not continue installation until you press a key.
	echo:
	pause
	echo:
...
if !HTTPSCERT_DETECTED!==n (
	...
	if not exist "server\the.crt" (
		...
		echo Vyond Legacy has nothing left to do since it can't launch without the.crt, so it will close.
		pause
		exit
	)
	:: Check for admin
	if /i not "!SAFE_MODE!"=="y" (
		...
		if /i not !ERRORLEVEL!==0 (
			if !VERBOSEWRAPPER!==n ( cls )
			echo For Vyond Legacy to work, it needs a HTTPS certificate to be installed.
...
if !ADMINREQUIRED!==y (
	...
	echo:
	echo Vyond Legacy no longer needs Admin rights,
...
title Vyond Legacy v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) [Loading...]
...
echo Vyond Legacy has been started^^! The video list should now be open.

::::::::::::::::
:: Post-Start ::
::::::::::::::::

title Vyond Legacy v!WRAPPER_VER! ^(build !WRAPPER_BLD!^)
...
echo:
echo Vyond Legacy v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) running
...
echo Enter 3 to open the settings
echo Enter 3 to open the server page
echo Enter ? to open the FAQ
echo Enter clr to clean up the screen
echo Enter "restart" to restart Vyond Legacy
echo Enter 0 to close Vyond Legacy
...
if /i "!choice!"=="deeee" echo DEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE & goto wrapperidle
if /i "!choice!"=="zurg" echo You Zurrrrrrrrrrrrrrrrrrrg & goto wrapperidle
if /i "!choice!"=="efkfkmmmm" echo efkfkfkfkfkfkfkfkfkfkfkfkmmmmmmmmmmmmmmmmmmmmmmmmmm & goto wrapperidle
:: dev options
...
if !INCLUDEDCHROMIUM!==n (
	if !CUSTOMBROWSER!==n (
		echo Opening Vyond Legacy in your default browser...
		start http://localhost:4343/pages/html/yourvideos.html
	) else (
		echo Opening Vyond Legacy in your set browser...
		start !CUSTOMBROWSER! http://localhost:4343/pages/html/yourvideos.html >nul
	)
) else (
	echo Opening Vyond Legacy using included Chromium...
	pushd utilities\ungoogled-chromium
	if !APPCHROMIUM!==y (
		start chrome.exe --user-data-dir=the_profile --app=http://localhost:4343/pages/html/yourvideos.html >nul
	) else (
		start chrome.exe --user-data-dir=the_profile http://localhost:4343/pages/html/yourvideos.html >nul
	)
	popd
)
goto wrapperidle
...
:open_files
pushd ..
echo Opening the Vyond Legacy folder...
start explorer.exe Vyond Legacy
popd
goto wrapperidle
...
echo watch benson on youtube
echo watch benson on youtube
echo watch evil benson on youtube
echo wa
goto wrapperidle
...
:exitwrapperconfirm
echo:
echo Are you sure you want to quit Vyond Legacy?
...
title Vyond Legacy v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) [Shutting down...]
...
echo Vyond Legacy has been shut down.
...
:exitwithstyle
title Vyond Legacy v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) [Shutting down... WITH STYLE]
